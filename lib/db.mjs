...
import { BEERS_DIR, LOCATIONS_DIR, BREWERIES_DIR, CHECKINS_DIR } from "./config.mjs";
...
// ── Checkin details (flavor profile) ────────────────────────────────────────

export function hasCheckinDetails(checkinId) {
  return checkinId ? hasEntity(CHECKINS_DIR, checkinId) : false;
}

export function readCheckinDetails(checkinId) {
  return checkinId ? readEntity(CHECKINS_DIR, checkinId) : null;
}

export function writeCheckinDetails(checkinId, data) {
  if (checkinId) writeEntity(CHECKINS_DIR, checkinId, data);
}
