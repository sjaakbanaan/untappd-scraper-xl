...
export function saveOutput(checkins, silent = false) {
  ...
  const enriched = sorted.map((c) => {
    ...
    const { brewery_url: _bru, failed: _brf, address: _bra, no_coords: _bnc, ...breweryData } = safeBrewery ?? {};
  ...
  if (!silent) {
    console.log(`${cArrow("➡")} Saved ${enriched.length} checkins ${cArrow("->")} ${OUTPUT_FILE}`);
  }
}
