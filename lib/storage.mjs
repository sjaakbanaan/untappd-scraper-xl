...
import { OUTPUT_DIR, OUTPUT_FILE, PROGRESS_FILE, USER } from './config.mjs';
import {
  readBeer,
  readLocation,
  readBrewery,
...
export function saveOutput(checkins, silent = false) {
  mkdirSync(OUTPUT_DIR, { recursive: true });
  ...
  const enriched = sorted.map((c) => {
    ...
    const purchasedDetails = c.purchased_at?.url
      ? readLocation(c.purchased_at.url)
      : null;
  ...
  if (!silent) {
    console.log(
      `${cArrow('➡')} Saved ${enriched.length} checkins ${cArrow(
        '->'
      )} ${OUTPUT_FILE}`
    );
  }
