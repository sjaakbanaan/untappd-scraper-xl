...
export function saveOutput(checkins) {
  ...
  const enriched = sorted.map((c) => {
    ...
    const { venue_url: _vu, failed: _vf, ...venueData }   = safeVenue    ?? {};
    const { brewery_url: _bru, failed: _brf, ...breweryData } = safeBrewery ?? {};
    const { venue_url: _pvu, failed: _pf, ...purchasedData } = safePurchased ?? {};

    // Also strip leaked keys from the base c objects (from a previous run's output)
    const { beer_url: _cbu, ...baseBeer } = c.beer ?? {};
    const { brewery_url: _cbru, ...baseBrewery } = c.brewery ?? {};
    const { venue_url: _cvu, ...baseVenue } = c.venue ?? {};
    const { venue_url: _cpvu, ...basePurchased } = c.purchased_at ?? {};
