...
export async function phase1(includeFlavors) {
  ...
  const flush = async (label) => {
    console.log('');
    console.log(
      cBatch(`💾 ${label} — flushing ${allCheckins.length} checkins...`)
    );
  ...
  try {
    while (true) {
      ...
      console.log(
        `   → ${checkins.length} on page | ${newOnThisPage} new | ${allCheckins.length} total`
      );
      const uniqueBeersOnPage = new Set(
        checkins.map((c) => c.beer?.url).filter(Boolean)
      ).size;
      console.log(
        `   Summary: ${newOnThisPage} new checkins added (Page ${pageNum}) | Total Checkins: ${uniqueBeersOnPage}`
      );
    ...
    const totalVenues = new Set(
      allCheckins
        .flatMap((c) => [c.venue?.url, c.purchased_at?.url])
        .filter(Boolean)
    ).size;
    ...
    console.log(
      cTotal(
        `🏆 Total Checkins: ${
          allCheckins.length
        } | Total Beers: ${totalBeers} | Total Venues: ${totalVenues} | Run Time: ${formatDuration(
          durationMs
        )}`
      )
    );
