...
export async function phaseIncremental(includeFlavors) {
  ...
  if (existsSync(OUTPUT_FILE)) {
    try {
      ...
      console.log(
        `📂 Loaded ${existingCheckins.length} existing checkins — scraping for new ones…\n`
      );
    } catch {
  ...
  try {
    while (!reachedKnown) {
      ...
      console.log(
        `📄  Page ${pageNum} ${
          cursor ? `(cursor: ${cursor})` : '(initial page)'
        }…`
      );
      ...
      console.log(
        `   → ${checkins.length} on page | ${newOnThisPage} new | ${newCheckins.length} total new`
      );
