...
export async function phaseUpdateAll() {
  ...
  try {
    ...
    console.error(
      `❌  Could not read ${OUTPUT_FILE}. Run a full scrape first.`
    );
    process.exit(1);
  }
  ...
  console.log(
    `📂  Loaded ${allCheckins.length} checkins from ${OUTPUT_FILE}\n`
  );
  ...
  console.log(
    `🍺  Update-All A: Re-fetching ${beerUrls.length} beer(s) [${CONCURRENCY} workers]…`
  );
  ...
  console.log(
    `\n💬  Update-All B: Re-fetching ${allCheckins.length} checkin(s) for toasts & comments [${CHECKIN_CONCURRENCY} worker(s)]…`
  );
