...
import { cPhase, cSuccess } from '../colors.mjs';
...
export async function phase3(allCheckins, newCheckins = null) {
  ...
  const venueUrls = [
    ...
    (u) => u.includes('/v/') && !SKIP_VENUE_URLS.has(u) && !hasLocation(u)
  );

  const breweryUrls = [
    ...new Set(allCheckins.map((c) => c.brewery?.url).filter(Boolean)),
  ].filter((u) => {
  ...
  try {
    ...
    if (venueUrls.length > 0) {
      ...
      console.log(cSuccess(`✅ ${vd} venues fetched, ${ve} errors`));
    }
    ...
    if (breweryUrls.length > 0) {
                ...
                if (!venueDetails || venueDetails.failed) {
                  try {
                    ...
                    if (venueDetails.lat != null && venueDetails.lng != null) {
                      const vGeo = await reverseGeocode(
                        venueDetails.lat,
                        venueDetails.lng
                      );
                      if (vGeo) {
                        ...
                        venueDetails.country =
                          vGeo.country || venueDetails.country;
                      }
                    }
      ...
      process.stdout.write('\n');
      console.log(cSuccess(`✅ ${bd} breweries fetched, ${be} errors`));
    }
  } catch (err) {
    process.stdout.write('\n');
    throw err;
