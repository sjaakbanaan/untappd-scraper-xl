...
export function parseBreweryNameSuffix(name) {
  ...
  for (let take = words.length - 1; take >= 1; take--) {
    ...
    if (KNOWN_COUNTRIES.has(candidate)) {
      return {
        city: words.slice(0, take).join(' ').trim(),
        state: '',
        country: candidate,
      };
    }
...
export function parseVenueDetails(html) {
  ...
  const name =
    $('.venue-detail h1, .venue-name h1, .venue h1, h1.title')
      .first()
      .text()
      .trim() ||
...
export function parseBreweryDetails(html) {
  ...
  const name =
    $(
      '.brewery-name h1, .brewery-detail h1, .brewery h1, h1.brewery-title, h1.title'
    )
      .first()
      .text()
      .trim() ||
...
export async function reverseGeocode(lat, lng) {
  ...
  try {
    ...
    country = CONSTITUENT_COUNTRIES.has(stateName)
      ? stateName
      : countryCtx?.text || '';
  ...
  for (const type of typeAttempts) {
    try {
      ...
      const url = `https://api.mapbox.com/geocoding/v5/mapbox.places/${encodeURIComponent(
        query
      )}.json?access_token=${MAPBOX_KEY}${typeParam}&limit=3&language=en`;
      const res = await fetch(url);
      if (!res.ok) continue;
      ...
      if (country) {
        bestFeature = data.features.find((f) => {
          ...
          if (
            UK_VARIANTS.includes(targetCountry) &&
        ...
        if (!bestFeature && type === 'country') {
          ...
          if (
            f.id.startsWith('country.') &&
            f.text.toLowerCase().includes(country.toLowerCase())
          ) {
            bestFeature = f;
          }
        }
