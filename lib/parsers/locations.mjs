...
export const KNOWN_COUNTRIES = new Set([
  ...
  "Turkmenistan", "Tuvalu", "Uzbekistan", "Vanuatu", "Yemen",
...
/**
 * Resolve lat/lng coordinates to { city, country } using Nominatim
 * (OpenStreetMap), which is free and requires no API key.
 *
 * - Always returns clean English country names (e.g. "Germany", "Netherlands")
 * - Uses the constituent country name (e.g. "Scotland", "Wales") instead of
 *   the sovereign state ("United Kingdom") when appropriate.
 * - Rate limit: 1 req/s — respected by Phase 3's existing DELAY_MS gap.
 * - Requires a User-Agent header (policy requirement).
 *
 * Returns { city, country } or null if the request fails.
 */
// Nominatim rate limiter: chain all requests so only one fires at a time,
// with a 1100ms gap between them (policy: max 1 req/s).
let _nominatimQueue = Promise.resolve();

export function reverseGeocode(lat, lng) {
  if (lat == null || lng == null) return Promise.resolve(null);

  // Each call is appended to the queue: wait for the previous call + delay,
  // then execute. Returns a Promise<{city, country}|null>.
  let resolve;
  ...
  _nominatimQueue = _nominatimQueue.then(async () => {
    ...
    for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
      try {
        ...
        const res = await fetch(url, {
          headers: { "User-Agent": "untappd-scraper-xl/1.0 (github.com/sjaakbanaan/untappd-scraper-xl)" },
        });

        if (res.status === 429) {
          // Rate limited — wait longer before retrying
          const backoff = attempt * 60_000;
          console.warn(`\n   ⏳ Nominatim rate limit (429) — waiting ${backoff / 1000}s before retry ${attempt}/${MAX_RETRIES}…`);
          await new Promise((r) => setTimeout(r, backoff));
          continue;
        }

        if (!res.ok) break; // non-retryable error

        const data = await res.json();
        const addr = data.address || {};
        const state   = addr.state || "";
        const country = CONSTITUENT_COUNTRIES.has(state) ? state : (addr.country || "");
    ...
    resolve(result);
    // Always wait 1100ms before the next queued request
