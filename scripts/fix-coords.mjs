...
const filePath =
  ...
  `./output/${process.env.UNTAPPD_USER}_checkins.json`;
...
async function main() {
  ...
  console.log(
    `🗺️   Scanning ${checkins.length} checkins for missing coordinates…\n`
  );
  ...
  if (CLEANUP && cleanupCount > 0) {
    console.log(
      `🧼  Cleaned up ${cleanupCount} country-only location(s) (reverted to null).\n`
    );
  }
  ...
  console.log(
    `  Found ${
      toFix.length
    } unique city/country pair(s) to resolve (affecting ${toFix.reduce(
  ...
  for (const item of toFix) {
    ...
    if (coords) {
      ...
      process.stdout.write(
        ` ✅ [${coords.lat.toFixed(4)}, ${coords.lng.toFixed(4)}] (updated ${
          objects.length
        } refs)\n`
      );
    } else {
