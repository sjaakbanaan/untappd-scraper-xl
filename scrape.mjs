...
async function main() {
  if (UPDATE_STATS) {
    if (INCLUDE_FLAVORS) {
      console.error(
        `❌  --include-flavors is not supported with --stats. Use 'npm run scrape' or 'npm run scrape:full' instead.`
      );
      process.exit(1);
    }
    console.log(
      `🔄 Untappd Scraper XL v${VERSION} — user: "${USER}" [--stats, concurrency: ${CONCURRENCY}]\n`
    );
  ...
  if (FULL_SCRAPE) {
    ...
    console.log(
      `🍺 Untappd Scraper XL v${VERSION} — user: "${USER}" [--full, concurrency: ${CONCURRENCY}${flags}]\n`
    );
    if (!INCLUDE_FLAVORS) {
      console.log(
        `ℹ️   Phase 4 (flavor profiles) skipped. Run with --include-flavors to enable it.\n`
      );
    }
  ...
  const flags = INCLUDE_FLAVORS ? ' +flavors' : '';
  console.log(
    `⚡ Untappd Scraper XL v${VERSION} — user: "${USER}" [incremental, concurrency: ${CONCURRENCY}${flags}]\n`
  );
